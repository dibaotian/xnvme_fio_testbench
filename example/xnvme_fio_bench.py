...
def run_fio_conf(job_id=None,filename=None,fio_size=None,fio_runtime=None):
    ...
    try:
        ...
        for index,item in enumerate(lines):
            if "write_iops_log" in item:
                if not os.path.exists(LOG_PATH+'/'+IOPS_LOG_PATH):
                    ...
                    output = subprocess.getoutput("cp plot_fio_bw_iops_log.py {}/{}/plot.sh".format(LOG_PATH,IOPS_LOG_PATH))
                    # print(output)
                    # output=subprocess.getoutput("ln -s plot_fio_bw_iops_log.py  {}/{}/plot.sh".format(LOG_PATH,IOPS_LOG_PATH))

        for index,item in enumerate(lines):
            if "write_bw_log" in item:
                if not os.path.exists(LOG_PATH+'/'+BW_LOG_PATH):
                    os.makedirs(LOG_PATH+'/'+BW_LOG_PATH)
                    # print("cp plot_fio_bw_log.py {}/{}/plot.sh".format(LOG_PATH,BW_LOG_PATH))
                    subprocess.getoutput("cp plot_fio_bw_iops_log.py {}/{}/plot.sh".format(LOG_PATH,BW_LOG_PATH))
                    # output=subprocess.getoutput("ln -s plot_fio_bw_iops_log.py  {}/{}/plot.sh".format(LOG_PATH,BW_LOG_PATH))

        for index,item in enumerate(lines):
            if "write_lat_log" in item:
                if not os.path.exists(LOG_PATH+'/'+LAT_LOG_PATH):
                    os.makedirs(LOG_PATH+'/'+LAT_LOG_PATH)
                    subprocess.getoutput("cp plot_fio_bw_iops_log.py {}/{}/plot.sh".format(LOG_PATH,LAT_LOG_PATH))
                    # output=subprocess.getoutput("ln -s plot_fio_bw_iops_log.py  {}/{}/plot.sh".format(LOG_PATH,LAT_LOG_PATH))
        ...
        # todo check if the device is exist
        for index,item in enumerate(lines):
            if 'filename=' in item:
                # print(item)
                conf_nvme_drv = item.split('=')[1].strip()
                if conf_nvme_drv not in nvme_drive_list:
                    print("the {} is not in the system".format(conf_nvme_drv))
                    raise Exception("nvme driver not exist")
...
def do_fio_job(job_id,filename=None,fio_size=None,fio_runtime=None):
    ...
    print(output)
    print("Start to process {}".format(job_id))
    ...
    try:
        ...
        logging.info('fio output format error {}'.format(output))
        # print ('FIO error: {}'.format(str(e)))
        print ('FIO error, please check the /tmp/xnvmebench.log')
...
if __name__ == '__main__':
    ...
    if os.geteuid() != 0:
        ...
        nvme_drive_list = subprocess.getoutput("ls /dev/nvme*n?")
        # print (nvme_drive_list.split("\n"))
